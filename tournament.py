...
from game_agent import custom_score, score_best_square, \
    score_with_centre_bias, score_best_square_with_unique_bias
...
def main():
    ...
    test_agents = [
        ...
        # Agent(CustomPlayer(score_fn=score_best_square, **CUSTOM_ARGS), "BestSquare"),
        # Agent(CustomPlayer(score_fn=score_with_centre_bias, **CUSTOM_ARGS), "CentreBias"),
        # Agent(CustomPlayer(score_fn=score_best_square_with_unique_bias, **CUSTOM_ARGS), "SmarterBestSquare"),
    ]
    ...
    for agentUT in test_agents:
        ...
        opponents = set(test_agents)
        opponents.remove(agentUT)
        opponents = list(opponents)
        agents = opponents + random_agents + mm_agents + ab_agents + [agentUT]
        win_ratio = play_round(agents, NUM_MATCHES)

        print("\n\nResults:")
        print("----------")
        print("{!s:<20}{:>10.2f}%".format(agentUT.name, win_ratio))
